...
proc help {} {
  ...
  puts "-hwts <T1,T2,...>        List of hardware threads to instantiate."
...
set hwts "rt_matrixmul,rt_matrixmul"
...
if { $::argc > 0 } {
  for {set i 0} {$i < [llength $::argc]} {incr i} {
    ...
    switch -regexp -- $option {
      ...
      "-hwts"      { incr i; set hwts       [lindex $::argv $i] }
      "-help"      { help }
      default {
        if { [regexp {^-} $option] } {
...
proc reconos_hw_setup {new_project_name new_project_path reconos_ip_dir hwts} {
    ...
    # Process $hwts which is a string that contains a comma seperated list of hardware
    # threads to implement
    set hwts_list [split $hwts ","]
    set hwts_num  [llength $hwts_list]
...
#    set_property -dict [list CONFIG.C_CLKIN_PERIOD {10.0}] [get_bd_cells reconos_clock_0]
    ...
    set_property -dict [list CONFIG.C_NUM_HWTS $hwts_num ] [get_bd_cells reconos_memif_arbiter_0]
    ...
    set_property -dict [list CONFIG.C_NUM_INTERRUPTS $hwts_num ] [get_bd_cells reconos_osif_intc_0]
    create_bd_cell -type ip -vlnv cs.upb.de:reconos:reconos_osif:1.0 reconos_osif_0
    set_property -dict [list CONFIG.C_NUM_HWTS  $hwts_num ] [get_bd_cells reconos_osif_0]
    create_bd_cell -type ip -vlnv cs.upb.de:reconos:reconos_proc_control:1.0 reconos_proc_control_0
    set_property -dict [list CONFIG.C_NUM_HWTS  $hwts_num ] [get_bd_cells reconos_proc_control_0]
    create_bd_cell -type ip -vlnv cs.upb.de:reconos:timer:1.0 timer_0

    # Add hardware thread and related FIFOs
    # TODO: hmm, usable hwts are hardcoded, make this more general
    for {set i 0} {$i < $hwts_num} {incr i} {
        set hwt [string trim [lindex $hwts_list $i]]
        switch -regexp -- $hwt {
        "rt_matrixmul" { create_bd_cell -type ip -vlnv cs.upb.de:reconos:rt_matrixmul:1.0 "rt_matrixmul_$i" }

        default {
            puts "ERROR: Unknown hardware thread '$hwt' specified!\n"
            return 1
            }
        }
        
        # Add FIFOS between hardware threads and arbiter
        create_bd_cell -type ip -vlnv cs.upb.de:reconos:reconos_fifo_sync:1.0 "reconos_fifo_osif_hw2sw_$i"
        create_bd_cell -type ip -vlnv cs.upb.de:reconos:reconos_fifo_sync:1.0 "reconos_fifo_osif_sw2hw_$i"
        create_bd_cell -type ip -vlnv cs.upb.de:reconos:reconos_fifo_sync:1.0 "reconos_fifo_memif_hwt2mem_$i"
        create_bd_cell -type ip -vlnv cs.upb.de:reconos:reconos_fifo_sync:1.0 "reconos_fifo_memif_mem2hwt_$i"
        
        # Set sizes of FIFOs
        set_property -dict [list CONFIG.C_FIFO_ADDR_WIDTH {3}] [get_bd_cells "reconos_fifo_osif_hw2sw_$i"]
        set_property -dict [list CONFIG.C_FIFO_ADDR_WIDTH {3}] [get_bd_cells "reconos_fifo_osif_sw2hw_$i"]
        
        set_property -dict [list CONFIG.C_FIFO_ADDR_WIDTH {7}] [get_bd_cells "reconos_fifo_memif_hwt2mem_$i"]
        set_property -dict [list CONFIG.C_FIFO_ADDR_WIDTH {7}] [get_bd_cells "reconos_fifo_memif_mem2hwt_$i"]
    }
    ...
    # OSIF <-> FIFOs <-> HWT
    connect_bd_intf_net [get_bd_intf_pins rt_matrixmul_0/OSIF_Hw2SW] [get_bd_intf_pins reconos_fifo_osif_hw2sw_0/FIFO_M]
    connect_bd_intf_net [get_bd_intf_pins rt_matrixmul_0/OSIF_Sw2Hw] [get_bd_intf_pins reconos_fifo_osif_sw2hw_0/FIFO_S]
    ...
    connect_bd_intf_net [get_bd_intf_pins rt_matrixmul_1/OSIF_Hw2SW] [get_bd_intf_pins reconos_fifo_osif_hw2sw_1/FIFO_M]
    connect_bd_intf_net [get_bd_intf_pins rt_matrixmul_1/OSIF_Sw2Hw] [get_bd_intf_pins reconos_fifo_osif_sw2hw_1/FIFO_S]
    ...
    # Arbiter <-> FIFOs <-> HWT
    connect_bd_intf_net [get_bd_intf_pins rt_matrixmul_0/MEMIF_Hwt2Mem] [get_bd_intf_pins reconos_fifo_memif_hwt2mem_0/FIFO_M]
    connect_bd_intf_net [get_bd_intf_pins rt_matrixmul_0/MEMIF_Mem2Hwt] [get_bd_intf_pins reconos_fifo_memif_mem2hwt_0/FIFO_S]
    ...
    connect_bd_intf_net [get_bd_intf_pins rt_matrixmul_1/MEMIF_Hwt2Mem] [get_bd_intf_pins reconos_fifo_memif_hwt2mem_1/FIFO_M]
    connect_bd_intf_net [get_bd_intf_pins rt_matrixmul_1/MEMIF_Mem2Hwt] [get_bd_intf_pins reconos_fifo_memif_mem2hwt_1/FIFO_S]
    ...
    # HWTs
    connect_bd_net [get_bd_pins reconos_clock_0/CLK0_Out] [get_bd_pins rt_matrixmul_0/HWT_Clk]
    connect_bd_net [get_bd_pins reconos_clock_0/CLK0_Out] [get_bd_pins rt_matrixmul_1/HWT_Clk]
    ...
    # Proc_control resets
    connect_bd_net [get_bd_pins reconos_proc_control_0/PROC_Hwt_Rst_0] [get_bd_pins rt_matrixmul_0/HWT_Rst]
    ...
    connect_bd_net [get_bd_pins reconos_proc_control_0/PROC_Hwt_Signal_0] [get_bd_pins rt_matrixmul_0/HWT_Signal]

    connect_bd_net [get_bd_pins reconos_proc_control_0/PROC_Hwt_Rst_1] [get_bd_pins rt_matrixmul_1/HWT_Rst]
    ...
    connect_bd_net [get_bd_pins reconos_proc_control_0/PROC_Hwt_Signal_1] [get_bd_pins rt_matrixmul_1/HWT_Signal]
...
reconos_hw_setup $proj_name $proj_path $reconos_ip_dir $hwts
